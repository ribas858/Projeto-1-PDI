function [Y] = preenche_pixels_brilho(Y, w, h)
  ...
  for i = 1 : (w * h)                             %% Percorre o vetor até seu final
    ...
    if Y(i) == 0                                  %% Se o valor da Y na posição i for zero
      if i - w >= 1 && intervalo == 2             %% Se o 'i-w' não gera uma posição invalida para o vetor 
        Y(i) = Y(i-w);                            %% Copia o valor da posição superior
      
      else i - 1 >= 1                             %% Se 'i-1' não gera uma posição invalida
        Y(i) = Y(i-1);                            %% Copia o valor da posiçõo anterior, a esquerda do pixel atual
      end
    end
